...
def graf_gistogram_poison(D, Q, t_step_min):
    df1 = D
    x1=np.array(df1['T'], dtype=int)
    # находит среднее значение P в заданном промежутке в зависимости от количества файлов
    Time_0 = np.array(df1['Time'], dtype=str)
...
def graf_gistogram_min_max_poison(D, Data_min, Data_max, t_step_min):
    df1 = D
    x1=np.array(df1['T'], dtype=int)
    # находит среднее значение P в заданном промежутке в зависимости от количества файлов
    Time_0 = np.array(df1['Time'], dtype=str)
    ...
    y1 = Data_min
    y2 = Data_max
    ax.step(x1, y1, color = 'r', linewidth = 2)
    ax.step(x1, y2, color = 'g', linewidth = 2)

    
    ax.set_xlabel('Время', size = 24)
    ax.set_ylabel('Минимальная и максимальная потребляема мощность по (распределению Пусассона) кВт ', size = 8)
    ax.tick_params(labelsize = 14)
    #ax.set_ylim(-50, max(y1)+1000)
    ax.set_xlim(-20, 1500)
    #ax.set_ylim(-50, 1500)
